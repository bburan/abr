...
def loadabr(fname, invert=False, filter=None, abr_window=8.5e-3):
    with tables.open_file(fname) as fh:
        ...
        signals = fh.root.waveforms[:, :, :cutoff]*1e6
        levels = fh.root.trial_log.read(field='level')
        frequency = fh.root.trial_log.read(field='frequency')[0]
        kw = dict(invert=invert, filter=filter)

        waveforms = [ABRWaveform(fs, signal, level, **kw)
                     for signal, level in zip(signals, levels)]
        series = ABRSeries(waveforms, frequency/1e3)
